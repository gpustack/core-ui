...
const AudioAnimation: React.FC<AudioAnimationProps> = (props) => {
  ...
  const startAudioVisualization = () => {
    ...
    const draw = (timestamp: number) => {
      ...
      for (let i = 0; i < barCount; i++) {
        const baseHeight =
          Math.floor((analyserData?.data?.[i] ?? 0) / 2) * scaleFactor;
  ...
  return (
    <div
      className={audioAnimationCss['scroller-wrapper']}
      ref={scrollerRef}
      style={{ width: '100%', height: '100%' }}
    >
      <div
        ref={containerRef}
        className={audioAnimationCss['canvas-wrap']}
        style={{ width: '100%', height: '100%' }}
      >
        <canvas
          ref={canvasRef}
          style={{ display: 'block' }}
          className={audioAnimationCss['canvas']}
        ></canvas>
      </div>
    </div>
  );
